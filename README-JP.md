...
3. **認証付きマルチユーザーモード**: `python server_auth.py -w ./work` を実行すると、X（Twitter）ログインとユーザーごとの閲覧/編集権限が使えます。下記「マルチユーザー・ログインサーバー」を参照。
...
## マルチユーザー・ログインサーバー (`server_auth.py`)

複数人で共有し、ユーザーごとに閲覧（または編集）できるタスクを限定したい場合は、`server.py` の代わりに `server_auth.py` を使います。同じエディタとエンドポイントを提供しつつ、以下を追加します：

* **X（Twitter）ログイン**：OAuth 2.0（認可コード + PKCE）、Cookie セッション。
* **ユーザーごとの権限**：`access.json` から読み込み。各 X ハンドルに `view` / `edit` のタスク一覧を設定（`"*"` = 全タスク；edit は view を含む；変更はホットリロード）。
* **サーバー側で強制される読み取り専用**：view ユーザーは読み取り専用エディタ（編集系コントロールは非表示、「読み取り専用」バッジを表示）になり、書き込みリクエストはクライアントに関わらず `403` で拒否されます。

```bash
export X_CLIENT_ID=...  X_CLIENT_SECRET=...  X_REDIRECT_URI=http://localhost:8000/auth/callback
cp access.json.sample access.json    # ユーザーを記入
python server_auth.py -w ./work -p 8000
```

ブラウザで `http://localhost:8000/` を開き → **Login with X** → 権限のあるタスクを選択。詳しいセットアップ（X アプリ、環境変数、権限ファイル、HTTPS デプロイ、権限モデル）は **[SERVER_AUTH.md](SERVER_AUTH.md)** および `server_auth.py` 冒頭の設定コメントを参照。`server.py` は引き続き認証なしの選択肢です。
