...
手動校正体験を向上させるために、軽量なウェブベースのビジュアルエディタ `editor.html` が用意されています。`pages.json` データを直接編集し、リアルタイムでレンダリング結果を確認できます。

- **リアルタイムプレビュー**: 実際のページ上で翻訳テキストがどのように見えるかを確認できます。
- **クイック編集**: サイドバーで翻訳を修正すると、キャンバスに即座に反映されます。
- **ズームとフィット**: 「最適サイズ」と「100% 表示」モードをサポートし、スクロールもスムーズです。
- **多言語 UI**: 日本語、英語、中国語の UI をサポートしています。
- **ショートカットキー**: `←`/`→` でページ移動、`Z` でズーム切替、`R` で再読み込み、`S` で保存。

### 使用方法:
1. ローカルサーバーを起動します (File System Access API のセキュリティ制限のため必要です):
   ```bash
   python -m http.server 8000
   ```
2. モダンなブラウザ (Chrome または Edge 推奨) で `http://localhost:8000/editor.html` を開きます。
3. **「Workディレクトリを開く」** をクリックし、`work` フォルダを選択して編集を開始します。
