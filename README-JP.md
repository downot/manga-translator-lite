...
`[ocr] ocr` は文字認識だけを切り替え、文字検出は変えません。`32px` / `48px` は認識器内で正規化する文字高であり、`detection_size` ではありません。まず `48px` を使い、OCR を切り替えた後は `extract --overwrite` でブロックを作り直してください。

| 値 | 向く場面 | トレードオフ |
|---|---|---|
| `"32px"` | CPU / VRAM が限られる場合の高速抽出、または明瞭で大きい印刷文字を大量に処理する場合。 | 旧来の 32 ピクセル自己回帰モデル。小文字、密集文字、装飾文字は抜けやすくなります。 |
| `"48px"` | 既定値。多言語の漫画・雑誌・一般ページにまず使う基準で、横書き/縦書きの通常テキスト向け。 | `32px` より少し計算量は増えますが、小さい文字の再現率は通常こちらが上です。 |
| `"48px_ctc"` | `48px` が特定文字を系統的に誤認識する時の比較用、または `ocr.ignore_bubble` で吹き出し外の自由文字 / SFX を除外する時。 | CTC デコーダーによる代替で、常に精度が上がるわけではありません。作品全体に使う前に代表ページで比較してください。 |
| `"mocr"` | 日本語中心の漫画、特に手書き・装飾的な日本語を内蔵認識器が誤読する場合。複数行の日本語を一つの句として読む必要がある時だけ `use_mocr_merge = true` を有効にします。 | 最も低速です。各領域で manga-ocr を追加実行しつつ、通常の形状/色の処理も行います。マージで隣接行が一つのブロックになるため、翻訳前に結果を確認してください。 |
